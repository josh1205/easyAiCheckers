...
class Checker(TwoPlayerGame):
    ...
    def __init__(self, players):
        """
        Setup of checkers game
        - assigns players to pieces
        - define's black pieces; white pieces
        - define's black territory; white territory
    ...
    def possible_moves_on_white_turn(self):
        """
        Iterate's through white's pieces on the board and appends playable moves to an array
        """
            ...
            for n in step_pos:
                ...
                if (n[0] >= 0 and n[0] <= 7) and (n[1] >= 0 and n[1] <= 7) and (n in black_squares):
                    ...
                    if board[n[0], n[1]] in ["B","W"]:
                        ...
                        # print("is_inside_board: ", is_inside_board)
                        if (j[0] <= 7) and (j[1] <=7):
                            is_position_empty = (board[j[0], j[1]] == 0)
                            # print("is_position_empy: ", is_position_empy)
                        else:
                        ...
                        if is_inside_board and (j in black_squares) and is_position_empty:
                            ...
                            # print("old_new_piece_pos: ", old_new_piece_pos)
                    else:
    ...
    def possible_moves(self):
        """
        (NOT optional)
        If the current player is equal to 2, show player 2's moves otherwise show player 1's moves
        """
    ...
    def get_piece_pos_from_table(self, table_pos):
        """
        param table_pos - position of all pieces on the (8 x 8) boards. type numpy array
    ...
    def lose(self):
        ...
        if self.current_player == 1:
            for i, j in self.white_territory:
                if self.board[i,j] == "B":
                    print("Black won, white has been defeated!")
                    return True
        else:
            for i, j in self.black_territory:
                if self.board[i,j] == "W":
                    print("White won, black has been defeated!")
                    return True
